...
- Bu nesnenin bâzı özellikleri şöyledir:
  ...
  - scripts : Nesne tipindedir. Çalıştırılacak...
...
> 
> 2. Durum bileşeninin geçmişini tutmak, önceki duruma dönmek; yeniden sonraki duruma dönmek gibi bir hâfıza yapısı daha kolay bir şekilde kurulabilir, bi iznillâh..
...
- Yukarıdaki bağlantı yönlendirmesine sâhip uygulamaya 'articles/1' bağlantı adresiyle bir istek gelirse `<Articles/>` bileşeni içerisinde bu parametreyi 'id' ismiyle alabiliriz, bi iznillâh..
...
- Sayfa yönlendirme linki oluşturmak için yine 'react-router-dom' altındaki 'Link' bileşenini kullanabiliriz, bi iznillâh. Gitmek istediğimiz adresi belirtmek için 'to' özelliğini kullanın:
...
- React Router'un fonksiyonla tanımlanmış bileşenleri için gelen adres bilgisini almak oldukça kolaydır, bi iznillâh..
...
- Eğer bu bileşenler fonksiyonla tanımlanmış bileşenler ise `useContext()` 'hook' fonksiyonunu kullanarak erişebiliriz, bi iznillâh! Bunun için ilgili bağlam bilgisini tanımladığımız dosyayı da sayfaya eklemeliyiz:
...
- Eğer bu birden fazla bağlam bilgisine sınıf içerisinde ihtiyacımız varsa, bu durumda uygulamamız gereken yöntem biraz daha karmaşıktır. En öz hâliyle bir fonksiyon oluşturup, bileşeni bu fonksiyona verip, dışarı aktarmaktır. Böylece sınıf içerisinden ilgili bağlam bilgilerine ulaşabilir olacağız bi iznillâh! kodlama örüntüsü şöyledir (koda çalışıyor, ama istenen sonuç alınmıyor, kontrol edilecek):
...
- Ezcümle Context API fonksiyon tanımlanan bileşenler için uygundur. Context API kullanımını iyileştirmek için bağlam bilgilerimizi de bağlamı tanımladığımız dosyaya alabiliriz, bi iznillâh! Fakat dikkat etmemiz gereken şeyler var. Öncelikle yeni bağlam tanım dosyamıza bakalım:
