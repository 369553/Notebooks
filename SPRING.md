...
- POJO (Persistent Object of Java...) (?) tabanlıdır.
...
- `getBean()` yönteminin ilk parametresi istenen nesnenin 'id' özelliğidir. İkinci parametresi ise istenen nesnenin hangi sınıftan bir nesne olarak istendiği bilgisidir. Eğer ikinci parametre verilmezse, istenen nesne 'Object' sınıfı örneği olarak gelir.
...
- Birden fazla yapıcı fonksiyon parametresi olduğunda bunların sırasının bir önemi yoktur; zîrâ 'name' özelliğiyle o yapıcı fonksiyon parametresinin hangi parametreye denk düştüğünü belirtiyoruz, bi iznillâh!
...
- Bunu yaptıktan sonra `<bean>` tanımı içerisine şu kodu ekleyerek, ilgili yapıcı fonksiyon girdi parametresini atanmasını sağlayabiliriz, bi iznillâh:
...
- Bunun için `org.springframework.core.env.Environment` sınıfından bir nesne oluşturmalıyız. Bu nesnenin üzerine `@Autowired` bildirimini ekleyerek nesnenin otomatik olarak ilklendirilmesini sağlayabiliriz, bi iznillâh!
...
- Spring'te 5 çeşit kapsam vardır:
  ...
  1. "singleton" (münferid) : Tanımlanan her fasulyeden ('bean') yalnızca bir tâne üretilir; gelen her talepte aynı nesne döndürülür. Varsayılan kapsamdır.
...
- Spring'te hem 'xml' ile, hem de bildirim (notasyon) ile sınıf kapsamını belirtebiliriz, bi iznillâh:
...
- Fakat bâzı durumlarda zerk (injection) işleminin otomatik olarak yapılmasını isteyebiliriz. Bildirim (notasyon) ile tanımlamada `@Autowired` bildirimini kullanarak bunu yapabiliriz, bi iznillâh:
...
- `@Service` : Hizmet veren yapılar, iş katmanları için bu bildirim kullanılır. Genellikle

- `@Repository` : 'bean' tanımlarının  yapıldığı yer olduğunu belirtmek için kullanılır(?)

- `@Controller` : Spring Web MVC'de gelen istekleri idâre eden, karşılayan katman olan 'Controller' katmanı olduğunu belirtmek için kullanılır.
