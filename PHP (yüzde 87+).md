...
- PHP, web siteleri ve web uygulamaları için kullanılan bir dildir.
...
- PHP'de her ifâdenin sonuna noktalı virgül konur.
...
- PHP'de değişkenin önüne parantez içinde 'bool' veyâ 'boolean' yazarak bitsel veriye çevrilebilen veri tipi tam sayıdır. Misal, "false" ifâdesini 'bool' tipine çevirmek istersek 'true' sonucunu elde ediyoruz!
...
- Geri döndürme işlemi ‘return' ifâdesiyle yapıyor.
...
- Yine fonksiyon içerisinden çağırabildiğimiz `func_get_arg($girdi_indeksi)` yöntemiyle ise verdiğimiz sıra numarasındaki fonksiyon girdi parametresine erişebiliriz, bi iznillâh
...
- Fonksiyonların girdi parametrelerini isimlendirerek, fonksiyon parametrelerinin sırasını bilmeden fonksiyonları çeşitli farklı biçimlerde kullanabiliriz, bi iznillâh. Bunlara 'named arguments' (isimlendirilmiş parametre) deniyor. Bu özellik PHP 8.0'dan itibâren varmış. Emsal kullanımı:
...
- 'null' değer 'string'e dönüştürüldüğünde boş string elde edilir.
...
- Bir fonksiyona çok farklı sayılarda parametre gönderilmesi isteniyorsa, bunun bir yolu fonksiyon imzasında herhangi bir parametre belirtmeyip, fonksiyon içerisinde `func_get_args()` yöntemini kullanarak gelen parametreleri almaktır.
  ...
  PHP 7 ile sayısız parametre işlecinin belirttiği değişkenin tipini belirleme özelliği de gelmiş buna göre fonksiyon imzasında girdi kısmına `int ... $sayilar` yazarak gelen tüm parametrelerin tam sayı tipinde olması gerektiğini belirtebilirsiniz, bi iznillâh.
...
- Dosya açma modları şunlardır:
  ...
  7. (?) Dosya hem okuma, hem de yazma modunda açılır; imleç dosya sonundadır, yazılan değerler dosya sonuna eklenir; dosya yoksa oluşturulur

##### Bu Alandaki Kullanışlı Bâzı Fonksiyonlar:

- `fread()` : dosya adresi ve mod bilgisi alır

- `filesize()` : Dosya adresi alır, dosya büyüklüğünü (?..) cinsinden geri döndürür
...
- JSON dosyaları `json_decode()` yöntemiyle parçalanıp, okunabilir, bi iznillâh
...
- PHP'de 'sadece okunabilir özellik' işlevi **8.1 versiyonuyla tanıtıldı**. Buna göre bir özelliğinin sadece bir kez değer alması sağlanabilir, bi iznillâh:
...
> **NOT - 1)**
> 
> Eğer sınıfın bir özelliği 'unset' ile silinirse, daha sonra o özelliğe erişilemez, hatâ verir.
> 
> **NOT - 2)**
